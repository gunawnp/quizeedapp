...
@section('content')
            ...
            <div class="card">
                <div class="card-body m-0">
                    @if ($dataQS != null)
                        <div class="row">
                            <div class="col">
                                <h6 class="m-0">Soal No. {{ $dataQS->number }}</h6>
                                <p>{{ $dataQS->question_text }}</p>
                            </div>
                            <div class="col d-grid d-sm-flex justify-content-md-end ">
                                <div>
                                    <form onsubmit="return confirm('Yakin mau hapus data?')" class="" action="/quiz_special/{{  $dataQS->material_id }}/{{ $dataQS->id }}" method="POST">
                                        @csrf
                                        @method('delete')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if (isset($data->quiz_special))
                        @foreach ($data->quiz_special as $item)
                            <a href="/quiz_special/{{$item->material_id}}/{{ $item->id }}" class="btn btn-outline-dark me-3 mt-3"> {{ $item->number }} </a>
                        @endforeach
                    @else
                        <p>- Data Kosong -</p>
                    @endif
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-header">
            ...
            <form action="/quiz_special" method="POST">
                @csrf
